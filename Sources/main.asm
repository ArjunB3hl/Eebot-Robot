...
NOT_RT_TRN      CMPA  #REV                      ; Else if it’s the REVERSE state            |
                BNE   NOT_REVERSE               ;                                           |
                JSR   REV_ST                    ; then call the REV_ST routine              |
                RTS                             ; and exit                                  |
;-----------------------------------------------;                                           |
NOT_REVERSE     CMPA  #SBY                      ; Else if it’s the STANDBY state            |
                BNE   NOT_SBY                   ;                                           |
...
NO_FWD_BUMP     BRSET PORTAD0,$08,NO_REV_BUMP    ; Else if D_DETN equals 1 then
                JSR   INIT_SBY                   ; Initialize the RT_TRN state
                MOVB  #SBY ,CRNT_STATE          ; Set CRNT_STATE to RT_TRN
                JMP   FWD_EXIT                
...
NO_RT_INTXN     LDAA  F_DETN                    ; Else if F_DETN equals 1
